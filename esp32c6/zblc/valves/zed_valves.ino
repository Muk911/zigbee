...
unsigned int relayPins[RELAY_COUNT] = {2, 4, 5, 6, 7, 19, 10, 3}; // для WT0132C6-S5 (ESP32-C6)
...
NvsVariable* nvValveState[VALVE_COUNT];
...
static void Valve_StateChanged(Valve *valve)
{
  ...
  nvValveState[i]->writeInt(state); // запоминаем новое состояние в NVRAM
...
void buttonClick(void)
{
  ...
  zaOnOff[1]->setValue((uint32_t) (zaOnOff[1]->getValue() ? 0 : 1));
  //uint8_t valveState;
  //nvValveState[0]->read(&valveState);
  //ESP_LOGI(TAG, "nvValveState: valveState %d", valveState);
  //valveState = (valveState ? 0 : 1);
  //nvValveState[0]->write(&valveState); // запоминаем новое состояние в NVRAM
}
...
void setup()
{
  ...
  for(int i = 0; i < VALVE_COUNT; i++) {
    ...
    nvValveState[i] = new NvsVariable(1, "valve_state", i + 1);
    uint8_t valveState = nvValveState[i]->readInt(-1);
    ...
    if (valveState) valves[i]->setState(0); // после перезагрузки отключаем включенные клапаны
  }
